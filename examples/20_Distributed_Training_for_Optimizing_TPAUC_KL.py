...
def evaluate(model, loader, device):
    ...
    if local_rank == 0:
        return pauc_roc_score(all_labels.numpy(), all_preds.numpy(), max_fpr=0.3, min_tpr=0.7)
    return None
...
loss_fn = tpAUC_KL_Loss(data_len=len(trainSet), Lambda=Lambda, tau=tau, gammas=(gamma0, gamma1), device=device)
optimizer = SOTAs(model.parameters(), loss_fn=loss_fn, mode='adam', lr=lr, weight_decay=weight_decay, device=device)
