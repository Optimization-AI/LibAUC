...
def evaluate(model, loader, device):
    ...
    if local_rank == 0:
        return auc_prc_score(all_labels.numpy(), all_preds.numpy())
    return None
...
for epoch in range(total_epoch):
   ...
   train_ap = evaluate(model, trainloader_eval, device)
   test_ap  = evaluate(model, testloader,       device)

   if local_rank == 0:
      train_loss_mean = np.mean(train_loss)
      if best_test < test_ap:
            best_test = test_ap
      print("epoch: %s, train_loss: %.4f, train_ap: %.4f, test_ap: %.4f, "
            "best_test_ap: %.4f, lr: %.4f"
            % (epoch, train_loss_mean, train_ap, test_ap, best_test, optimizer.lr))
